...
def adoption_logs(url):
    response = requests.get(url)
    if response.status_code == 200:
        return response.text
    else:
        print(f"Не получилось получить данные: HTTP {response.status_code}")
        return None
...
def insert_to_db(db_cursor, log_data):
    db_cursor.execute('''
